...
let gravitySpeed;//constant vel going downward
let slideSpeed;//constant vel going downward
let risingSpeed=1;//amount gas is allowed to rise upward per frame
let flowSpeed;//current flow speed of water
let flowChance;
...
addEventListener("message",(d)=>{
    //console.log(d);
    //intialize variables
    blocks=d.data.physicsData.blocks;
    movedAlready=d.data.physicsData.movedAlready;
    gravitySpeed=d.data.physicsData.gravitySpeed;
    slideSpeed=d.data.physicsData.slideSpeed;
    flowSpeed=d.data.physicsData.flowSpeed;
    flowChance=d.data.physicsData.flowChance;
    randoms=d.data.physicsData.randomInts;
    //run physics
    physics();

    //return data to be used by draw
    let message={physicsDataResult:{blocks:blocks,movedAlready:movedAlready}}
    postMessage(message);
});

//transferred from library to here  for ease of use
let posRand=0;
...
for(let a=0;a<gravitySpeed;a++)
{
    ...
    {	
        if(interactionCheck(blocks[i+(z+a)*cols],blocks[i+(z+1+a)*cols],0,1,i,z,0,a)==true){}//done inside interaction check
        else{
            let temp=blocks[i+(z+1+a)*cols];
            ...
            for(let a=0;a<slideSpeed;a++)
            {
                if(z+1+a<rows&&i+1+a<cols)
                {
                    ...
                    interactionCheck(blocks[(i+a)+(z+a)*cols],blocks[(i+1+a)+(z+1+a)*cols],1,1,i,z,a,a)){
                        if(interactionCheck(blocks[(i+a)+(z+a)*cols],blocks[(i+1+a)+(z+1+a)*cols],1,1,i,z,a,a)){}
                        else{
                            let temp=blocks[(i+1+a)+(z+1+a)*cols];
...
if(movedAlready[i+z*cols]==0)
{
    ...
    if(right)
    {
        for(let a=0;a<flowSpeed;a++)
        {
            ...
            {	
                if(interactionCheck(blocks[(i+a)+z*cols],blocks[(i+1+a)+z*cols],1,0,i,z,a,0)==true){}
                else{
                    let x=getRandomInt(0,flowChance);
                    if(x==0){
                        let temp=blocks[(i+1+a)+z*cols];
    ...
    {
        for(let a=0;a<flowSpeed;a++)//check based on a
        {
...
if(movedAlready[i+z*cols]==0)
{
    ...
    if(z-1>=0)
    {
        ...
        if(rightDiag)
        {
            ...
            {	if(interactionCheck(blocks[i+z*cols],blocks[(i+1)+(z-1)*cols],1,-1,i,z,0,0)){}
                else{
                    let temp=blocks[(i+1)+(z-1)*cols];
...
//does a lot of checks so made it seperate
    ...
    for(let x=-1; x<=1;x++)//check surroundings//x=-1;x<=1;x++
    {
        for(let y=1; y>=1;y--)//y=1;y>=-1;y--
        {
            ...
            {    
                ...
                else if((iD1==10||iD1==11||iD1==12)&&temp==7)//if fire oil
                {
                    blocks[(i+x)+(z+y)*cols]=10;
                    blocks[(i)+(z)*cols]=10;
                    movedAlready[(i)+(z)*cols]=1;
